...
public class SigSignerJavaImpl implements SigSigner {
    ...
    public void sign(@NonNull HttpRequestBase request) throws Exception {
        ...
        for (Header header : request.getAllHeaders()) {
            headerMap.put(header.getName(), List.of(header.getValue()));
        }
        byte[] bytes;
        if (HttpPost.METHOD_NAME.equals(request.getMethod())) {
            bytes = ((HttpPost) request).getEntity().getContent().readAllBytes();
    ...
    public SignVo sigSign(@NonNull SignQo signQo) {
        ...
        HttpHeaders authHeader = addAuthorityHeaders(signQo.getHeaders(), authorizationStr);
    ...
    private void addCommonHeaders(SignQo signQo, String signDate) {
        HttpHeaders headers = signQo.getHeaders();
        Map<String, List<String>> map = headers.map();
        HashMap<String, List<String>> newMap = new HashMap<>(map);
        List<String> sdkContentType;
        if (CollectionUtils.isEmpty(headers.allValues(Constants.CONTENT_TYPE))) {
            sdkContentType = List.of("application/json");
        } else {
            sdkContentType = headers.allValues(Constants.CONTENT_TYPE);
        }
        newMap.put(Constants.SDK_CONTENT_TYPE, sdkContentType);
        newMap.put(Constants.SDK_HOST, List.of(signQo.getHost()));
        newMap.put(Constants.SIGN_DATE, List.of(signDate));

        signQo.setHeaders(HttpHeaders.of(newMap, (key, value) -> true));
    }
     ...
     * @return 添加过后的请求头
     */
    private HttpHeaders addAuthorityHeaders(HttpHeaders headers, String authorizationStr) {
        Map<String, List<String>> map = headers.map();
        HashMap<String, List<String>> newMap = new HashMap<>(map);
        newMap.put(Constants.SIG_HEADER_AUTHORIZATION, List.of(authorizationStr));
        return HttpHeaders.of(
                newMap,
                (key, value) -> {
                    List<String> list = List.of(
                            Constants.SIG_HEADER_AUTHORIZATION,
                            Constants.SDK_CONTENT_TYPE,
                            Constants.SDK_HOST,
                            Constants.SIGN_DATE
                    );
    ...
    private void addSdkHeaders(HttpRequestBase request, SignVo signVo) {
        final HttpHeaders headers = Objects.requireNonNull(signVo.getAuthorityHeader());
        final Map<String, List<String>> headersMap = headers.map();
    ...
    private String getSignedHeaders(HttpHeaders headers) {
        List<String> keys = headers.map().keySet().stream().sorted(
     ...
     * @return 签名时间
     */
    private String getSignDate(HttpHeaders headers) {
        return headers.firstValue(Constants.SIGN_DATE).orElse(DateFormatUtils.formatUTC(new Date(), Constants.DATE_FORMAT));
    }
     ...
     * key:value;
     */
    private String getHeaderStr(HttpHeaders headers) {
    ...
    public static String sigEScape(String srcStr, int type) {
        ...
        String encodedStr = URLEncoder.encode(srcStr, StandardCharsets.UTF_8);
